...
if (str (getMarkerPos "TOUR_SI_marker_1") == "[0,0,0]") then
{
	...
	{
		if ((_commandType != "Return To Base")&&(_commandType != "Toggle Engine")) then
		{
			_text = 
			"
...
if (_text2 < 1) then
{
	...
	if ((_supportType == "Helicopter")&&(_commandType != "Toggle Engine")) then
	{
		_text = 
		"
			<t size='0.85' font='TahomaB' color='#D0D0D0' align='left' valign='top'>FLIGHT HEIGHT NOT SELECTED: Please select the height in which you want the helicopter to fly at</t>
			<br/>
		";
		_abort = true;
	};
...
if (((_supportType == "Helicopter")or(_supportType == "Plane"))&&(_commandType != "Toggle Engine")) then
{
