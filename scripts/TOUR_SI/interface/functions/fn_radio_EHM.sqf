...
if (_type == "toggle engine") then 
{
		if (!isNil {(_unitarray select 1) getVariable "TOUR_SI_unitOrders"}) then 
		{
			_height = (_unitarray select 1) getVariable "TOUR_SI_unitOrders" select 2;
			_radius = (_unitarray select 1) getVariable "TOUR_SI_unitOrders" select 3;
			_speed = (_unitarray select 1) getVariable "TOUR_SI_unitOrders" select 4;
			_behaviour = (_unitarray select 1) getVariable "TOUR_SI_unitOrders" select 5;		
		}else 
		{
			_behaviour = "hold fire";
			_speed = "LIMITED";
			_height = 100;
			_radius = 100;
		};
...
if (!isDedicated) then
{
	...
	sleep 3;
	if ((_type == "return to base")or(_type == "toggle engine")) then
	{
		_strPos = "Base";
	}else
	{
	...
	if (alive (leader (_unitarray select 0))) then
	{
		switch (toLower _type) do
		{
			...
			case "toggle engine":	{
										if (isEngineOn (_unitArray select 1)) then 
										{
											_unit sideChat format ["%2, can you disengage your engines? OVER", _groupstring, _groupunit, _strPos];
										}else 
										{
											_unit sideChat format ["%2, can you engage your engines? OVER", _groupstring, _groupunit, _strPos];
										};
									};						
		};
	};
